...
function revelarCartaAleatoria() {
    ...
    if (!tabuleiro)
        return;
    const cartasNaoViradas = Array.from(tabuleiro.children).filter((carta) => carta.innerText === '?');
    if (cartasNaoViradas.length === 0)
        return;
    const cartaAleatoria = cartasNaoViradas[Math.floor(Math.random() * cartasNaoViradas.length)];
